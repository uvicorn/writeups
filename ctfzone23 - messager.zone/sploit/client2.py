...
start_time = 1689544348#int(time.time())
...
class Ratchet:
    ...
    def RatchetEncrypt(self, plaintext, AD):
        ...
        dec_flag = DECRYPT(mk, enc, CONCAT(AD, header))
        print('decrypted', dec_flag)
        return header, enc
...
class Server:
    ...
    def store_message(self, from_m, to_m, header, message):
        ...
        # if ('12496372874031488032606048532478096688305166692422034810339216599759166940092123800166060840684408803643251125396829875455370968705492774497117713529043766' not in data['header']) and ('1440713984198861418851534104440707381044434672349657514491537207645264915887352699279171998733642947910433339738407681660592901758411812697271345614143122' not in data['header']):
        # if '9587505793080140103524847811583274244050120566603216235210265422430128194131336715399761531868205642343274143146150353115177652759091221841855204403054045' in data['header']:
        if '10007557092155308918620703319005578742802155531542215927047486636897385017421026687658302411228112816312151375158019561914808361055666669789420151300043193' in data['header']:
            print('FOUND', start_time)
            # exit()
...
def start_main(comb):
    ...
    for i in range(len(choses)):
        ...
        if choose == '1':
            ...
            msg = 'ABOBA'+os.urandom(4).hex()#input('Msg: ')
...
# for i in itertools.product([0, 1], repeat=7):
    random.seed(start_time)
    start_main(list([i])+[0,])
    print('NOW', start_time, 1689544414-start_time)
    print('-'*150)
    start_time-=1
print('WASTED', time.time()-rt)